...
- **특징**: METT-C 요소를 종합적으로 고려하여 최적의 대응 방책을 제안하며, 7대 핵심 지표(위협대응, 자원가용성, 전력능력, 환경적합성, 효과성, 연계성, 임무부합성)를 기반으로 정밀한 점수를 산출합니다.

### 2. 온톨로지 스튜디오 (Ontology Studio)
전술 지식 체계(Ontology)를 시각적으로 관리하고 검증하는 도구입니다.
- **기능**: 클래스/속성 탐색, SPARQL 쿼리 실행, 관계 시각화, 무결성 검증, **Inference Rule 실행**
- **특징**: 복잡한 RDF 데이터를 그래프 형태로 직관적으로 표현하고, 전술적 규칙을 통해 새로운 관계를 자동으로 추론합니다.

### 3. 지식 그래프 (Knowledge Graph)
구축된 온톨로지 데이터를 기반으로 심층적인 관계를 탐색합니다.
- **기능**: 노드 중심 탐색, 연관 개체 추적, 추론 경로 확인, **군사 교리(Doctrine) 연동**
- **특징**: 단순 데이터 조회를 넘어 '왜'라는 질문에 답할 수 있는 인과적 정보를 제공하며, RAG 검색 결과와 연계하여 근거를 제시합니다.

### 4. 데이터 관리 (Data Management)
16종의 전술 마스터 데이터와 RAG 지식 베이스를 관리합니다.
- **기능**: 데이터 CRUD, 품질 지표 대시보드, **RAG 문서 업로드 및 벡터화**, 이상치 탐지
- **특징**: 데이터와 교리 문서를 통합 관리하여 AI 추론의 정확도와 투명성을 보장합니다.
...
## 🚀 운영 서버 배포 가이드 (Operational Deployment)

운영 환경 이관 및 안정적 운영을 위해 다음 절차를 권장합니다.

### 1. 환경 설정 (.env)
루트 디렉토리에 `.env` 파일을 생성하고 필수 정보를 설정합니다.
```env
...
npm run build
```
- 빌드 결과물(`frontend/dist`)은 Nginx 등으로 서빙하거나, FastAPI에서 정적 파일 폴더로 마운트하여 사용할 수 있습니다.

### 3. 백엔드 운영 실행 (Gunicorn + Uvicorn)
운영 환경에서는 멀티 프로세스 관리를 위해 Gunicorn 사용을 적극 권장합니다.
```bash
# 가상환경 활성화 후 실행
...
### 4. 데이터 무결성 점검
- 배포 전 `scripts/validate_data_quality.py`를 실행하여 16개 테이블의 오류 여부를 최종 확인하십시오.
- `instances_reasoned.ttl` 파일을 미리 생성해두면 운영 시 초기 로딩 속도가 크게 개선됩니다.
```
