...
| 요소 | 기본 가중치 | 설명 |
|------|--------|------|
| 위협 대응 | 0.20 | 가장 중요 (정상 상황 시 20%, 위협 고조 시 최대 45%) |
| 임무 부합성 | 0.20 | 부여된 임무 목적과의 일관성 |
| 자원 가용성 | 0.15 | 작전 수행에 필요한 자산 및 부대 확보 여부 |
| 전력 능력 | 0.12 | 투입되는 아군의 화력, 사기 등 전투 지수 |
| 환경 적합성 | 0.12 | 지형 및 기상 조건 부합도 |
| 효과성 | 0.12 | 유사 과거 사례 기반의 성공률 |
| 연계성 | 0.09 | 전술적 전개 및 후속 조치와의 유기적 관계 |

### 3.2 상황 적응형 가중치 (Adaptive Weights)

시스템은 고정된 가중치를 넘어 상황의 맥락에 따라 가중치를 동적으로 재분배합니다.

1. **Survival Mode (위협 수준 >= 0.8)**:
   - 위협 대응 가중치 상향 (0.20 → **0.45**)
   - 부대 및 자산 확보가 위협 제거보다 후순위가 됨

2. **Goal Mode (임무 명확성 높음)**:
   - 임무 부합성 가중치 상향 (0.20 → **0.40**)
   - 상급 부대 지침 이행을 최우선으로 평가
