...
### 2.2 관계 및 점수 추출
- **`_extract_threat_score()`**: 상황도 기반 위협 수준 추출 및 정규화
- **`_extract_resource_availability()`**: 온톨로지 SPARQL 쿼리를 통한 자원 매칭률 산출
- **`_extract_environment_fit()`**: 지형/기상 조건과 작전 간의 시맨틱 호환성 검사
- **`calculate_dynamic_score()`**: 지형(산악, 도시, 평원 등) 및 기상 효과를 반영한 동적 점수 보정
